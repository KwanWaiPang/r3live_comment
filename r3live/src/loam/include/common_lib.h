...
struct Camera_Lidar_queue
{
    ...
    bool if_lidar_can_process()//看看lidar是否可以被处理
    {
        // m_if_have_lidar_data = 1;
        double cam_last_time = get_camera_front_time();//获取camera+imu后的时间
        double lidar_last_time = get_lidar_front_time();//获取lidar+imu后的时间
        if (m_if_have_camera_data == 0)//如果有camera data是0？就是没有camera数据，可以处理？？？
        {
            return true;
        }

        if (cam_last_time < 0 || lidar_last_time < 0)//camera跟lidar都还没有，那么就不处理
        {
        ...
        if (lidar_last_time > cam_last_time)//由于需要先处理camera的，所以如果lidar的时间大于camera的，就是在camera后发出的。则不处理？？？？
        {
...
struct MeasureGroup // Lidar data and imu dates for the curent process（存了lidar跟imu）
{
    MeasureGroup()
    {
